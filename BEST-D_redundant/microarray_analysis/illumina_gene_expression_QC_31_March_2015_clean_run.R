...
# These may not work downstream! e.g. neqc() requires an EListRaw class as input (ie what is output from read.ilmn.targets) and
# produces an EList but normaliseIllumina() requires an eset as input and outputs xxx. PAIN! To avoid this use the package 'convert':
...
#Then read all files:
# PAIN! If 'ctrlfiles=NULL' is specified it will not create the $genes$Status column which
...
# 
# TO DO: The package quantro tests whether quantile normalisation is appropriate for the given dataset:
...
# Variance-stabilising transformation from the 'lumi' package (VSN normalise then VST transform):
# ?vsn2, ?normalize.quantiles, ?normalizeVSN
...
normalize_VSN_background_corrected <- normalizeVSN(background_corrected)
class(normalize_VSN_background_corrected)
head(normalize_VSN_background_corrected)
dim(normalize_VSN_background_corrected)
range(normalize_VSN_background_corrected$E)
head(normalize_VSN_background_corrected$E)

# Other methods for normalisation (require object as ExpressionSet so transform data object):
#transform_vst <- normaliseIllumina(minimal_eset, transform='vst')
#run_vsn <- normaliseIllumina(minimal_eset, transform='vst', method='vsn')
#run_vsn_2 <- normaliseIllumina(lumi_read_data, method='vsn')
...
# normalize_VSN
# normalize_VSN_background_corrected
...
meanSdPlot(normalize_VSN_background_corrected$E, main='Background corrected, normalised expression values - VSN')
par(mfrow=c(1,1))
dev.off()
...
limma::plotMA(normalize_VSN_background_corrected$E, main='Background corrected, normalised expression values - VSN')
par(mfrow=c(1,1))
dev.off()
...
plotDensity(normalize_VSN_background_corrected$E, logMode=F, addLegend=F, 
            main='Density distribution of background corrected, normalised expression values - VSN')
par(mfrow=c(1,1))
dev.off()
...
# 
# head(t(normalised_expressed$E[-1]), 5)
# 
# #?prcomp
# 
...
# plot(pca_normalised_expressed)
# 
# # Check how much of the variance in gene expression values is explained by the first x PCs:
# sum(pca_normalised$sdev[1:10]^2)/length(normalised_expressed$E)
...
# treatment status, etc.) to be able to create a design and contrast:
