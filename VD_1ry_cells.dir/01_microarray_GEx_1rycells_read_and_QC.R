...
membership_input_file <- 'phenodata.txt'
membership_input_file

# TO DO: pass as file:
# List of samples that fail QC:
failed_QC_input_file <- c()
...
boxplot(log2(read_files$E),range=0,ylab="log2 intensity", xlab="Array")
dev.off()
...
#Get clean EListRaw object. If no samples are excluded this will remove the headers:
#read_files_cleaned_QC <- read_files[,-to_extract]

read_files_cleaned_QC <- read_files
...
sample_IDs_clean <- colnames(read_files_cleaned_QC)
length(sample_IDs_clean)

#Cross IDs:
matched_IDs <- which(membership_file$sample %in% sample_IDs_clean)
length(matched_IDs)
...
#############################
# Determine which samples to exclude and output list of sample names for next script as above from ArrayQualityMetrics (aqm) output.
# This next run for aqm is more of a sanity check as samples to exclude have already been identified.
# TO DO: delete this and only run summary plots for before/after comparison.
