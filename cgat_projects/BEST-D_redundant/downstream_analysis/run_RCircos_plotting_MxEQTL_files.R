...
# cis_file <- 'biomart_annot_cut_genotype_data_all_treated_baseline.tsv_matched.tsv_MxEQTL_p5_1MB.cis.txt'
# cis_file <- 'biomart_annot_cut_genotype_data_all_treated_final.tsv_matched.tsv_MxEQTL_p5_1MB.cis.txt'

# eQTL trans file to place as ribbon lines with link plot:
trans_file <- as.character(args[2])
# trans_file <- 'biomart_annot_cut_genotype_data_all_treated_baseline.tsv_matched.tsv_MxEQTL_p8_1MB.trans.txt'
# trans_file <- 'biomart_annot_cut_genotype_data_all_treated_final.tsv_matched.tsv_MxEQTL_p8_1MB.trans.txt'
...
cis_data_pvalues <- cis_data[, c("SNP_chr_name", "SNP_chrom_start", "SNP_chrom_end", "p.value")]
head(cis_data_pvalues)
dim(cis_data_pvalues)
# Convert to -log10
cis_data_pvalues$log10_pvalues <- -log10(cis_data_pvalues[, 4])
...
links_plot_file_trans <- trans_data[, c("SNP_chr_name", "SNP_chrom_start", "SNP_chrom_end", 
                                          "Probe_ID_chromosome_name", "Probe_ID_start_position", "Probe_ID_end_position")]
...
colnames(Gex_data)
# Select columns for use:
Gex_data <- Gex_data[, c(14, 15, 16, 4, 3)] # Had a weird error for heatmap which asked for column 5
head(Gex_data)
dim(Gex_data)
...
# Sort to get the top hits by pvalue:
highlight_genes_data <- highlight_genes_data[order(highlight_genes_data$FDR, highlight_genes_data$beta), ]
head(highlight_genes_data)
# Select columns:
head(highlight_genes_data)
colnames(highlight_genes_data)
