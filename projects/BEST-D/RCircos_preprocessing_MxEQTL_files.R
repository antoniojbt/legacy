...
# setwd('/Users/antoniob/Desktop/BEST_D.DIR/scripts_to_upload/RCircos_plotting_2/')
...
R_session_saved_image <- paste('R_session_saved_image_RCircos_preprocessing_MxEQTL','.RData', sep='')
...
column_to_use_SNP <- as.character(args[2])
# column_to_use_SNP <- 'SNP'

column_to_use_SNP <- as.character(args[3])
# column_to_use_ProbeID <- 'gene' # As MatrixEQTL output
####################


####################
# Read data:
Mx_eQTL_data <- fread(Mx_eQTL_file, sep = '\t', header = TRUE, stringsAsFactors = FALSE, 
                         strip.white = TRUE, na.strings = c('', ' ', 'NA', '-', 'na'))
class(Mx_eQTL_data)
str(Mx_eQTL_data)
head(Mx_eQTL_data)
dim(Mx_eQTL_data)
Mx_eQTL_data <- as.data.frame(Mx_eQTL_data)
####################
...
host_name <- 'www.ensembl.org'

# Use SNP position as first set of coordinates:
# Check what's available:
listMarts()
# Check what datasets are included in the biomart database:
mart <- useMart(biomart = 'ENSEMBL_MART_SNP', host = host_name)
listDatasets(mart)
# Set the source (database) and dataset to use:
snpmart <- useMart(biomart = 'ENSEMBL_MART_SNP', dataset = 'hsapiens_snp', host = host_name)
...
snp_ids <- Mx_eQTL_data[, c(column_to_use_SNP)]
head(snp_ids)
length(snp_ids)
...
# Set the source (database) and dataset to use:
ensembl <- useMart("ensembl", dataset="hsapiens_gene_ensembl", host = host_name)
# Check what information is available
...
# Get the values:
probe_IDs <- Mx_eQTL_data[, c(column_to_use_ProbeID)]
head(probe_IDs)
length(probe_IDs)
...
# For normal MatrixEQTL files:
# file_name <- sprintf('biomart_annot_%s.txt', Mx_eQTL_file)
# cmd_run <- sprintf('cat %s | cut -f7-9,13-15 > RCircos_%s', file_name, file_name)
# system(cmd_run)
####################
