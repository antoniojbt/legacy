...
topTable_results <- as.character(args[1])
# topTable_results <- 'full_topTable_pairing_all_treated.txt'
# topTable_results <- 'cis_data_joint_data2_spec_annot.tsv'
p_value_cut_off <- as.numeric(args[2])
# p_value_cut_off <- 0.05
q_value_cut_off <- as.numeric(args[3])
# q_value_cut_off <- 0.05

topTable_results <- read.csv(topTable_results, header = T, sep = '\t')
head(topTable_results)
tail(topTable_results)
dim(topTable_results)
# colnames(topTable_results)[1] <- 'probe_ID'
...
top_genes <- get_EntrezIDs_top_genes(topTable_results, p_value_cut_off)
# top_genes <- topTable_results$ENTREZ_ID
head(top_genes)
length(top_genes)
...
# TO DO: pass fit object with coef column and entrezID column, see manual, so as to add up and down reg genes.
...
# Effect size can be the genes from the diff. exp. set provided (DE) with N (total genes in the term), eg:
(topG0_top_genes_MF$DE/topG0_top_genes_MF$N)*100
...
ggplot(topG0_top_genes_BP, aes(y=((DE/N)*100), x=Term)) + 
  geom_point(stat='identity', aes(colour = P.DE, size = DE)) + 
  ylab('% genes contained in the set (DE)') +
  xlab('Term') + 
  coord_flip()
dev.off()

png('barplot_topGO_MF.png', width = 12, height = 12, units = 'in', res = 300)
ggplot(topG0_top_genes_MF, aes(y=((DE/N)*100), x=Term)) +
  geom_point(stat='identity', aes(colour = P.DE, size = DE)) + 
  ylab('% genes contained in the set (DE)') +
  xlab('Term') + 
  coord_flip()
...
ggplot(topG0_top_genes_CC, aes(y=((DE/N)*100), x=Term)) +
  geom_point(stat='identity', aes(colour = P.DE, size = DE)) + 
  ylab('% genes contained in the set (DE)') +
  xlab('Term') + 
  coord_flip()
...
# kegga_top_genes <- kegga(top_genes, universe=ENTREZID_universe, plot = TRUE, species='Hs')
# head(kegga_top_genes)
# topKEGG_top_genes <- topKEGG(goana_top_genes, number=50, ontology='BP')
...
clusterP_enrich_MF <- enrichGO(gene=top_genes, universe=ENTREZID_universe, pAdjustMethod='BH', ont='MF', minGSSize=10,
                     pvalueCutoff=p_value_cut_off, qvalueCutoff=q_value_cut_off, readable=T, organism='human')
...
clusterP_enrich_BP <- enrichGO(gene=top_genes, universe=ENTREZID_universe, pAdjustMethod='BH', ont='BP', minGSSize=10,
                               pvalueCutoff=p_value_cut_off, qvalueCutoff=q_value_cut_off, readable=T, organism='human')
...
clusterP_enrich_CC <- enrichGO(gene=top_genes, universe=ENTREZID_universe, pAdjustMethod='BH', ont='CC', minGSSize=10,
                               pvalueCutoff=p_value_cut_off, qvalueCutoff=q_value_cut_off, readable=T, organism='human')
...
# TO DO: errors:
# clusterP_gse <- gseGO(geneList = top_genes, ont = 'GO', ENTREZID_universe, 
#                       pAdjustMethod='BH', pvalueCutoff=p_value_cut_off, nPerm=100, organism='human')
# head(summary(clusterP_gse))

clusterP_kegg <- enrichKEGG(gene=top_genes, universe=ENTREZID_universe, pvalueCutoff=p_value_cut_off, 
                            qvalueCutoff=q_value_cut_off, pAdjustMethod='BH', organism='human')
head(summary(clusterP_kegg))
barplot(clusterP_kegg)

# TO DO: these error:
#clusterP_gse_kegg <- gseKEGG(gene=top_genes_1_ENTREZID, pvalueCutoff=p_value_cut_off, nPerm=100, organism='human')
#head(summary(clusterP_kegg))

# clusterP_DAVID <- enrichDAVID(gene=top_genes_ENTREZID, idType="ENTREZ_GENE_ID", listType='Gene', 
#                               species = 'human', pvalueCutoff=p_value_cut_off, 
#                               qvalueCutoff=q_value_cut_off, pAdjustMethod='BH')
