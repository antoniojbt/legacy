...
source('/Users/antoniob/Documents/github.dir/cgat_projects/projects/BEST-D/functions_for_MatrixeQTL.R')
########################
...
# eQTL_file_name <- '2000+4000-12months-1-VS-2000+4000-baseline-1.reQTL_cis'
...
# snp <- "rs6435757"
# snp <- 'rs2282679' # VDBP chr4 SNP

probe <- as.character(args[5])
# probe <- "ILMN_1807425"
# probe <- 'ILMN_21791' # GC (VDBP Illumina probe, see Illumina download file)
# probe <- 'ILMN_8966' # CYP2R1 Illumina probe, see Illumina download file
# probe <- 'ILMN_163752'	# CYP24A1
# as.data.frame(probe_file[probe, ])
...
colnames(snp_probe_data)
str(snp_probe_data)
snp_probe_data[, snp] <- as.numeric(as.character(snp_probe_data[, snp]))
...
snp_probe_data[, probe] <- as.numeric(as.character(snp_probe_data[, probe]))
str(snp_probe_data)
...
str(snp_probe_PC_data)
snp_probe_PC_data[, snp] <- as.numeric(as.character(snp_probe_PC_data[, snp]))
# TO DO: check whether to run as additive (numeric encoding) or logistic (factor encoding):
# snp_probe_PC_data[, snp] <- as.factor(as.numeric(as.character(snp_probe_PC_data[, snp])))
snp_probe_PC_data[, probe] <- as.numeric(as.character(snp_probe_PC_data[, probe]))
str(snp_probe_PC_data)
...
sapply(snp_probe_data, class)
# lm_fit <- lm(formula = ILMN_1807425 ~ rs6435757, data = snp_probe_data)
...
sapply(snp_probe_PC_data, class)
# pass_formula <- as.formula(sprintf('%s ~ %s + .', probe, snp))
pass_formula <- as.formula(sprintf('%s ~ .', probe, snp))
pass_formula
...
lm_summary_coef[snp, ]
lm_summary_PCs_coef[snp, ]
eQTL_data[which(eQTL_data[, SNP] == snp), ]


# TO DO: save to file
# file_name <- sprintf('%s_%s_%s_%s_%s.pdf', snp, probe, gene_name, plot_file_name, plot_file_name_group)
# 
...
plot_title <- sprintf('Effect of %s on expression levels of %s (%s)', snp, probe, gene_name)
plot_title
...
ggplot(snp_probe_data, aes(as.factor(snp_probe_data[, snp]), snp_probe_data[, probe])) + 
  geom_jitter(colour='darkgrey', position=position_jitter(width=0.25)) + 
  geom_boxplot(outlier.size=0, alpha=0.6, fill='grey') + 
  ylab('Average gene expression levels') + 
...
# If SNP numeric:
gex_corrected <- snp_probe_PC_data[, probe] - rowSums(coef(lm_fit_PCs)[-(1:2)] * snp_probe_PC_data[, 3:ncol(snp_probe_PC_data)])
# Collect as data frame to pass to ggplot2:
gex_corrected_snp_df <- data.frame(expression = gex_corrected, genotype = factor(snp_probe_PC_data[, snp]))
ggplot(gex_corrected_snp_df, aes(genotype, expression)) +
  geom_jitter(colour="darkgrey", position=position_jitter(width=0.25)) +
  geom_boxplot(outlier.size=0, alpha=0.6, fill="grey") + 
  ylab('Average adjusted gene expression levels') + 
  xlab('Genotype') + ggtitle(plot_title) + theme_minimal() +
  theme(text = element_text(size = 8))
