...
# setwd('/Users/antoniob/Desktop/BEST_D.DIR/scripts_to_upload/')
...
# geno_file <- 'test_cut_genotype_data_all_treated_baseline.tsv_matched.tsv'

# Gene expression levels:
Gex_file <- as.character(args[2])
# Gex_file <- 'cut_GEx_baseline_4000_and_2000.tsv_matched.tsv'
# Gex_file <- 'test_cut_GEx_baseline_4000_and_2000.tsv_matched.tsv'

snpspos = as.character(args[3])
genepos = as.character(args[4])
...
choose_covariates = chooseCov(expression = Gex_file, genotype = geno_file, covariate = covOut_filename, 
                              ...
                              doCis = TRUE, doTrans = TRUE, candidates = PC_seq_to_test,
...
trans_file <- choose_covariates$best[[2]]
trans_file
trans_file_path <- sprintf('%s%s', base_path, trans_file)
cmd_soft_link <- sprintf('ln -s %s trans_%s', trans_file_path, dirname(trans_file))
cmd_soft_link
...
qqplot_file <- paste(choose_covariates$best[[2]], '.pdf', sep = '')
qqplot_file
qqplot_file_path <- sprintf('%s%s', base_path, qqplot_file)
cmd_soft_link <- sprintf('ln -s %s qqplot_%s_trans.pdf', qqplot_file_path, dirname(qqplot_file))
cmd_soft_link
system(cmd_soft_link)
...
ggplot(data = as.data.frame(choose_covariates), aes(y = eqtls.significant.trans, x = covariates)) +
  geom_point() +geom_line() + 
  xlab('Gene expression principal components') + ylab('Unique probes with associated trans SNPs')
